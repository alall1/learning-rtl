...
#endif  // VL_DEBUG
void Vaccumulator___024root___eval_static(Vaccumulator___024root* vlSelf);
...
void Vaccumulator::eval_step() {
    ...
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        ...
        Vaccumulator___024root___eval_static(&(vlSymsp->TOP));
...
//============================================================
// Events and timing
bool Vaccumulator::eventsPending() { return false; }

uint64_t Vaccumulator::nextTimeSlot() {
...
void Vaccumulator___024root___eval_final(Vaccumulator___024root* vlSelf);

VL_ATTR_COLD void Vaccumulator::final() {
    contextp()->executingFinal(true);
    Vaccumulator___024root___eval_final(&(vlSymsp->TOP));
...
void Vaccumulator::prepareClone() const { contextp()->prepareClone(); }
void Vaccumulator::atClone() const {
...
void Vaccumulator___024root__trace_decl_types(VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vaccumulator___024root__trace_decl_types(tracep);
...
VL_ATTR_COLD void Vaccumulator::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vaccumulator::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP), name(), false, 30);
    Vaccumulator___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
